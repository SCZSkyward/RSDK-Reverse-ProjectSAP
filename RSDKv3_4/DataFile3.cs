...
namespace RSDKv3
{
    public class DataFile : IDataFile
    {
        ...
        public bool FileExists(string fileName) => files.Any(a => a.fullFilename.Equals(fileName));

        public byte[] GetFileData(string fileName) => files.First(a => a.fullFilename.Equals(fileName)).fileData;

        public bool TryGetFileData(string fileName, out byte[] fileData)
        {
            var f = files.FirstOrDefault(a => a.fullFilename.Equals(fileName));
            fileData = f?.fileData;
            return f != null;
        }
    }
