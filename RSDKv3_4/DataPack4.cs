...
namespace RSDKv4
{
	public class DataPack : IDataPack
    {
        ...
        public void Read(Reader reader, List<string> fileNames = null)
        {
            ...
            {
                reader.Close();
                throw new Exception("Invalid DataPack v4 signature");
            }
		...
		private byte[] GetHash(string str) => MD5Hasher.GetHash(new ASCIIEncoding().GetBytes(str.ToLowerInvariant()));

		public bool FileExists(string fileName)
        {
            byte[] hash = GetHash(fileName);
            return files.Any(a => a.name.hash.SequenceEqual(hash));
        }

        public byte[] GetFileData(string fileName)
        {
            byte[] hash = GetHash(fileName);
            return files.First(a => a.name.hash.SequenceEqual(hash)).data;
        }

        public bool TryGetFileData(string fileName, out byte[] fileData)
        {
            byte[] hash = GetHash(fileName);
            var f = files.FirstOrDefault(a => a.name.hash.SequenceEqual(hash));
            fileData = f?.data;
            return f != null;
        }
    }
