...
namespace RSDKv3
{
    public class ArcContainer : IDataPack
    {
        public class File
        {
            /// <summary>
        ...
        public bool FileExists(string fileName) => files.Any(a => a.name.Equals(fileName));

        public byte[] GetFileData(string fileName) => files.First(a => a.name.Equals(fileName)).data;

        public bool TryGetFileData(string fileName, out byte[] fileData)
        {
            var f = files.FirstOrDefault(a => a.name.Equals(fileName));
            fileData = f?.data;
            return f != null;
        }
    }
