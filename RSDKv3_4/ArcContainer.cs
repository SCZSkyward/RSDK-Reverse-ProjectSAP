...
namespace RSDKv3
{
    public class ArcContainer : IDataFile
    {
        ...
        public bool FileExists(string fileName) => files.Any(a => a.fileName.Equals(fileName));

        public byte[] GetFileData(string fileName) => files.First(a => a.fileName.Equals(fileName)).fileData;

        public bool TryGetFileData(string fileName, out byte[] fileData)
        {
            var f = files.FirstOrDefault(a => a.fileName.Equals(fileName));
            fileData = f?.fileData;
            return f != null;
        }
    }
