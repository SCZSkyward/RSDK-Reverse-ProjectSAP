...
namespace RSDKv3
{
    public class DataPack : IDataPack
    {
        ...
        public bool FileExists(string fileName) => files.Any(a => a.fullName.Equals(fileName));

        public byte[] GetFileData(string fileName) => files.First(a => a.fullName.Equals(fileName)).data;

        public bool TryGetFileData(string fileName, out byte[] fileData)
        {
            var f = files.FirstOrDefault(a => a.fullName.Equals(fileName));
            fileData = f?.data;
            return f != null;
        }
    }
